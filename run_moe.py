...
# tf.disable_eager_execution()  # Ensure TensorFlow 1.x compatibility
...
def main3(config):
    ...
    # Ensure the same session is used
    sess = tf.Session(config=c)
    K.set_session(sess)
    K.clear_session()  # Reset any existing sessions
    tf.reset_default_graph()  # Clear any previous graph references
    ...
    # Choose Model
    model = None
    deep_ctr_lora_list = ['mlp_lora', 'wdl_lora', 'nfm_lora', 'autoint_lora', 'deepfm_lora', 'dcn_lora', 'xdeepfm_lora',
                          'fibinet_lora', 'pnn_lora']
    deep_ctr_list = ['mlp_single', 'wdl_single', 'nfm_single', 'autoint_single', 'deepfm_single', 'dcn_single',
                     'xdeepfm_single', 'fibinet_single', 'pnn_single']
    ...
    # 1) Train ONLY the backbone
    ###########################################################################
    freeze_dict = {"backbone": False, "gate": True, "all_experts": True, "expert_index": None}
    freeze_mlora_parts(model, freeze_dict)

    for layer in model.model.layers:
        if isinstance(layer, MloraMoE):
            layer.use_gate = False
            layer.expert_index = None

    # FIX: Reset graph before recompiling
    K.clear_session()
    tf.reset_default_graph()  # Ensure clean computation graph
    sess = tf.Session(config=c)  # New session
    K.set_session(sess)
    model.model.compile(loss=model.train_config['loss'], optimizer=opt,
    ...
    for i in range(num_experts):
        print("Training expert", i)
        freeze_dict = {"backbone": True, "gate": True, "all_experts": None, "expert_index": i}
        freeze_mlora_parts(model, freeze_dict)
        ...
        domain_id = i % model.n_domain
        model.reset_early_stop()
        model.train(domain_ids=[domain_id])
    ...
    # 3) Train ONLY the gate
    ###########################################################################
    freeze_dict = {"backbone": True, "gate": False, "all_experts": True, "expert_index": None}
    freeze_mlora_parts(model, freeze_dict)
